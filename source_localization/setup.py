...
dtypes = Extension(
    ...
    library_dirs=["/usr/lib64/"],   # get_path_to_file(find("libQtCore.so*", "/")[0])],
    libraries=["QtCore"],
    include_dirs=[numpy.get_include(), "/usr/lib64/qt5/include/QtCore/"]  #get_path_to_file(find("qstring.h", "/usr")[0]), './dlib/', './'],
)
...
setup(ext_modules=cythonize([dtypes])) # ,  lh, est]))
