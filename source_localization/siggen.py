...
def wgn_baseband(antenna, target, length, fs, **keywords):
    ...
    antenna can be dtypes.Antenna() object or dtypes.AntennaSystem() object.
    ...
    if type(antenna) is source_localization.Antenna:
        sys = source_localization.AntennaSystem()
        sys.append(antenna)
        antenna = sys

    output = []
    for ant_counter in range(len(antenna)):
        ant = antenna[ant_counter]
        channels_total = ant.get_channels_total()
        elements = ant.get_elements()
...
def rimp(antenna, target, length, fs, pulse_width, period, carrier, **keywords):
    ...
    period = int(period * fs)
    pulse_width = int( pulse_width * fs)
    ...
    for ant_counter in range(len(antenna)):
        ...
        for p in range(int(np.ceil(length / period))):
            for i in range(int(np.floor(pulse_width))):
                if (i + p * period) >= length:
                    break
                s0[i + p * period] += 1 * np.cos(2 * np.pi * carrier * i / fs)
